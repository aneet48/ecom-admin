  ...
  <div>
    ...
    <div class="text-right" v-if="parent_data">
      <v-btn
        ...
        color="teal accent-4"
        dark
        :to="`/events-categories/${parent_data.parent_id}`"
      >
...
export default {
  ...
  methods: {
    ...
    deleteItem(item) {
      this.$swal({
        ...
        if (result.value && item) {
          this.overlay = true;
          axios
            .post(`api/event-category/delete/${item.id}`)
            .then(res => {
              let data = res.data;
    ...
    saveDialog() {
      ...
      let api_url =
        this.m_type == "edit"
          ? `api/event-category/${this.editItem.id}`
          : "api/event-category";
      axios({
        method: "post",
        url: api_url,
    ...
    fetchCategories() {
      ...
      this.overlay = true;
      axios
        .get(`/api/event-categories/true/${cat_id}?page=${this.currentPage}`)
        // .get("/api/event-categories/true/cat_id?page=" + this.currentPage)
        .then(res => {
          this.overlay = false;
          this.loader = false;
